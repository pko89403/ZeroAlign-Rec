...
> 기술 스택·주요 모듈·빌드/검증 명령·Repo-local Codex Commands 등 도메인 구체 정보는 [`references/local-adaptation.md`](references/local-adaptation.md)를 참조한다.
...
> 언어 규칙은 [`references/local-adaptation.md`](references/local-adaptation.md)의 "Language Conventions"를, graphify orchestration 세부는 [`.agents/skills/graphify-manager/SKILL.md`](.agents/skills/graphify-manager/SKILL.md)를 참조한다.
