...
| taxonomy | `src/sid_reco/taxonomy/` | taxonomy step 1 임베딩/FAISS 검색, taxonomy dictionary 생성, item-level taxonomy structuring (`item_projection.py`) |
| cli | `src/sid_reco/cli.py` | `doctor`, `smoke-mlx`, `smoke-llm`, `smoke-embed`, `prepare-foodcom`, `build-taxonomy-step1`, `build-taxonomy-dictionary`, `structure-taxonomy-item`, `structure-taxonomy-batch` |
