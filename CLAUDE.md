...
> 언어 규칙은 [`.agents/policies/local-adaptation.md`](.agents/policies/local-adaptation.md)의 "Language Conventions"를, upstream `/graphify` public surface는 [`.agents/skills/graphify/SKILL.md`](.agents/skills/graphify/SKILL.md)를, 기본 corpus boundary는 [`.graphifyignore`](.graphifyignore)를 참조한다.
...
- 교차 모듈("A와 B가 어떻게 연결되는가" 유형) 질문은 grep 대신 `graphify query "<질문>"`, `graphify path "<A>" "<B>"`, `graphify explain "<개념>"`을 우선 사용한다. EXTRACTED + INFERRED 엣지를 활용해 파일 스캔보다 정확하다.

> 이 저장소는 upstream `/graphify` 사용 모델을 따른다. repo-local Graphify split이나 staged flow는 문서 계약으로 유지하지 않으며, 기본 입력 제외 규칙은 `.graphifyignore`로 관리한다.
