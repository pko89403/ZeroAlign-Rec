...
repo-local `/graphify` skill이 단일 public Graphify entrypoint다.
이 저장소는 별도 repo-local Graphify split이나 staged refresh contract를 문서화하지 않고,
upstream `/graphify` mental model을 그대로 따른다.

assistant integration 기준선은 upstream Graphify의
`graphify claude install`, `graphify codex install`, `graphify opencode install`,
`graphify copilot install` 계열을 따른다.

`graphify hook install`, `graphify hook status`, `--watch` 같은 surface는
upstream Graphify capability로 읽고, 이 README에서 별도 repo-local Graphify workflow 계약으로 해석하지 않는다.
...
기본 `/graphify .` 입력 경계는 `.graphifyignore`로 관리한다.
이 저장소는 `AGENTS.md`, `CLAUDE.md`, 최상위 README들, `SPEC.md` 같은 repo 메타 문서와
`graphify-out/` 생성물, `apps/demo/screenshots/` UI 스크린샷,
`assets/branding/` 브랜딩 자산, `tests/fixtures/graphify/` 같은 Graphify 전용 fixture를
명시적으로 지정하지 않는 한 기본 입력에서 제외한다.
...
그래프에 필요한 source 문맥이 부족하면 `raw/`를 직접 확인한다.
