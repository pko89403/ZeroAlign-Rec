...
def test_write_sid_index_outputs_persists_faiss_and_mapping_artifacts(tmp_path: Path) -> None:
    ...
    compiled = CompiledSIDItems(
        items=[
            CompiledSIDItem(recipe_id=101, sid_path=(0, 1), sid_string="<0>-<1>"),
            CompiledSIDItem(recipe_id=102, sid_path=(1, 0), sid_string="<1>-<0>"),
            CompiledSIDItem(recipe_id=103, sid_path=(0, 1), sid_string="<0>-<1>"),
        ],
        branching_factor=2,
        depth=2,
        embedding_dim=3,
...
def test_write_sid_index_outputs_rejects_misaligned_recipe_ids(tmp_path: Path) -> None:
    ...
    compiled = CompiledSIDItems(
        items=[CompiledSIDItem(recipe_id=999, sid_path=(0,), sid_string="<0>")],
        branching_factor=2,
        depth=1,
        embedding_dim=2,
        levels=(
    ...
    with pytest.raises(ValueError, match="Embedded items and compiled SID items must align"):
