...
def test_git_hooks_delegate_to_shared_scripts_with_repo_root_fallback() -> None:
    ...
    compatibility_pre_commit = (ROOT / "scripts" / "git-hooks" / "pre-commit.sh").read_text(
        encoding="utf-8"
    )
    compatibility_pre_push = (ROOT / "scripts" / "git-hooks" / "pre-push.sh").read_text(
        encoding="utf-8"
    )
    ...
    assert "scripts/hooks/pre-commit.sh" in compatibility_pre_commit
    assert "scripts/hooks/pre-push.sh" in compatibility_pre_push
